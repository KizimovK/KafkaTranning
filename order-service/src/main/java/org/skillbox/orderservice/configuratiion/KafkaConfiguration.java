package org.skillbox.orderservice.configuratiion;
...
public class KafkaConfiguration {
    ...
    @Value(value = "${app.kafka.kafkaGroupId}")
    private String kafkaOrderGroupId;

    @Bean
    public ProducerFactory<UUID, OrderEvent> producerFactory(ObjectMapper objectMapper){
    ...
    public KafkaTemplate<UUID, OrderEvent> kafkaTemplate(
            ProducerFactory<UUID, OrderEvent> producerFactory){
        return new KafkaTemplate<>(producerFactory);
    }

    @Bean
    public ConsumerFactory<UUID, OrderStatusEvent> consumerFactory(ObjectMapper objectMapper) {
        Map<String, Object> config = new HashMap<>();
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<UUID, OrderStatusEvent> kafkaListenerContainerFactory(
            ConsumerFactory<UUID, OrderStatusEvent> consumerFactory){
        ConcurrentKafkaListenerContainerFactory<UUID, OrderStatusEvent> factory =
